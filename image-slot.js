...
(() => {
  ...
  const stylesheet =
    ...
    // touch-action:none only while actively reframing (dragging the crop) —
    // this is a design-tool authoring widget; on a deployed/read-only page
    // data-editable and data-reframe never get set, so leaving this
    // unconditional on the base image silently blocked page scroll on touch
    // over every content image site-wide (this component is used for nearly
    // all of them). The reframe drag itself lives on .spill (own touch-action
    // below), so this rule only matters for the underlying clipped image
    // during that same interactive state.
